...
- **系统信息**
  ...
  - 主板 SN、系统 SN、UUID
  - 有线/无线网卡（含型号与 MAC 地址）、蓝牙地址

- **系统状态**
  - Windows 版本、Build、系统架构、安装日期、最近启动时间和运行时长
  - Windows 激活状态、系统语言、时区、Windows 目录
  - Secure Boot 和 BitLocker 状态
  - 计算机名、域/工作组、登录用户、系统类型
  - 总/可用物理内存、虚拟内存总计/可用、页面文件
  - 引导设备、Hyper-V 状态、HAL 版本、电源计划

- **系统异常**
  - 展示设备管理器中 `ConfigManagerErrorCode <> 0` 的异常驱动/设备信息
  - 查询最近 30 天 System 日志中的重大异常事件：WER SystemErrorReporting 1001、System Error 1003、System Error/Kernel-Power 41
  - 展示事件时间、来源、类型、记录号、计算机名和详细消息
...
- **驱动详情**
  - 三级树形结构：设备类别 → 设备名称 → 驱动版本/日期/提供商
  - 数据来源于 `Win32_PnPSignedDriver`，按 36 个常见设备类分组
  - 支持展开/折叠，默认展开类别和设备两级
  - 替代原"硬件详情"页面
...
- **实用工具**
  - 内置 6 款硬件检测工具（CPU-Z、BatteryInfoView、Core Temp、HWiNFO64、CrystalDiskMark、USBTreeView）
  - 以图标卡片形式展示，首次点击自动释放到程序目录并运行，后续直接启动
  - AMD / Nvidia 驱动下载链接，点击以默认浏览器打开官方驱动页面
...
- 支持最大化、最小化和窗口缩放
- 侧边栏共 11 个导航项：系统信息、系统状态、系统异常、开机自启、驱动详情、SSD信息、屏幕详情、电池日志、电源日志、实用工具、系统设置
- 程序启动后先显示系统信息，再静默预加载 SSD 信息、屏幕详情、系统异常、电池日志和电源日志，切换页面时更流畅
- 耗时信息均使用后台线程加载，加载期间界面保持响应
- 电池日志和电源日志由 `powercfg` 导出为 HTML，保存在程序 EXE 所在目录，启动预加载或刷新都会覆盖同名报告文件
- 内置工具首次点击自动释放至程序目录，后续直接运行，无需重复释放
...
| Core Temp | CPU 温度与负载监控 |
| HWiNFO64 | 全方位硬件检测与监控 |
| CrystalDiskMark | 磁盘读写性能基准测试 |
| USBTreeView | USB 设备树查看器 |
